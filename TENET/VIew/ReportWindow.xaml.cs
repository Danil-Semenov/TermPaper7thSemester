...
namespace TENET
{
    ...
    public partial class ReportWindow : ReactiveWindow<ReportViewModel>
    {
        ...
        public ReportWindow()
        {
            ...
            string sql = "SELECT [id_проект] as 'Номер-проекта', [Название] as 'Проект', Дата_Начала as 'Дата Начала', [Дата_сдачи], [Предмет_договора] as 'Предмет договора' FROM dbo.Проект WHERE [Название] IS NOT NULL AND [Предмет_договора] IS NOT NULL AND [Дата_Начала] IS NOT NULL AND [Дата_сдачи] IS NOT NULL";
